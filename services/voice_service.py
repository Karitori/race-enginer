...
from utils.text import strip_markdown_code_fences
...
class VoiceAssistant:
    ...
    def __init__(self):
        ...
        provider = os.getenv("VOICE_PROVIDER") or os.getenv("LLM_PROVIDER")
        model = os.getenv("VOICE_MODEL") or os.getenv("LLM_MODEL")
        self.summary_client = ChatClient(provider=provider, model=model, temperature=0.2)
        if not self.summary_client.available:
            logger.warning("voice summarizer LLM not configured. Smart summarizing disabled.")
...
"""
            try:
                ...
                text_response = await self.summary_client.generate_text(
                    "You are an F1 race engineer voice summarizer.",
                    prompt,
                )
                if not text_response:
                    continue
                text_response = strip_markdown_code_fences(text_response)
